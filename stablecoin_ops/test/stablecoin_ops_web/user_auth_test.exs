defmodule StablecoinOpsWeb.UserAuthTest do
  use StablecoinOpsWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias StablecoinOps.Accounts
  alias StablecoinOps.Accounts.Scope
  alias StablecoinOpsWeb.UserAuth

  import StablecoinOps.AccountsFixtures

  @remember_me_cookie "_stablecoin_ops_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, StablecoinOpsWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      StablecoinOpsWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: StablecoinOpsWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        StablecoinOpsWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
